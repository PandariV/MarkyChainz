class Terrain {
    constructor(difficulty) {
        ...
        for (var i = 100; i < 1000; i+=100) {
            this.coordinates.push([this.x + i,  height - noise(i, car.chasis.position.x, random(100)) *100 - 40]);
            this.vertices.push(Matter.Vector.create(this.x + i,  height - noise(i, car.chasis.position.x, random(100)) *100 - 40));
        }
        this.coordinates.push([this.x+1000, this.y]);
        this.coordinates.push(Matter.Vector.create(this.x+1000, this.y));

        this.actualVertices = Matter.Vertices.create(this.vertices, this.body);
